...
contract Cryptopolis {
    ...
    event BalanceChanged(address player, uint256 newBalance, int256 payment);
    ...
    function buyTicket(string _name) payable public {
        ...
        emit BalanceChanged(msg.sender, initialFunds, int256(initialFunds));
    ...
    function _arrival(uint256 playerId, uint256 position) private {
    ...
    function _getChance(uint256 _fieldId, uint256 _playerId) private {
        ...
        if (chanceCards[cardNumber] == CHANCE_FEE) {
            ...
            !_sellAssets(_playerId, CHANCE_FEE_AMOUNT)) {
                ...
                emit BalanceChanged(player.addr, player.balance, -int256(CHANCE_FEE_AMOUNT));
            } else {
            ...
            emit BalanceChanged(player.addr, players[_playerId].balance, int256(CHANCE_REWARD_AMOUNT));
    ...
    function _payFee(uint256 _fieldId, uint256 _playerId) private {
        ...
        int256 payment = (-field.payment) * int256(multiplier);
        if (int256(player.balance) - payment < 0 &&
        !_sellAssets(_playerId, uint256(payment))) {
            _playerLost(_playerId);
            emit BalanceChanged(player.addr, player.balance, -payment);
        } else {
            ...
            if (field.owner != EMPTY_OWNER) {
                ...
                emit BalanceChanged(fieldOwner.addr, fieldOwner.balance, payment);
            }
        }
    }
    ...
    function _downgradeField(uint256 _fieldId) private {
        ...
        emit FieldUpdated(
            _fieldId, field.payment, field.owner, field.level, field.fieldType, field.group, field.price, field.buyingRight);
        emit BalanceChanged(player.addr, player.balance, int256(field.price / 2));
    }
    ...
    function _buyField(uint256 _fieldId) private {
        ...
        emit FieldUpdated(
            _fieldId, field.payment, field.owner, field.level, field.fieldType, field.group, field.price, field.buyingRight);
        emit BalanceChanged(player.addr, player.balance, -int256(field.price));
    }
