...
contract Cryptopolis {
    ...
    constructor() public {
        ...
        fields[9] = Field({payment: 100, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_FEE, group: 0, price: 0, buyingRight: EMPTY_OWNER});
        ...
        fields[15] = Field({payment: 100, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_FEE, group: 0, price: 0, buyingRight: EMPTY_OWNER});
        ...
        fields[25] = Field({payment: -3, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_COMPANY, group: 6, price: 170, buyingRight: EMPTY_OWNER});
        fields[26] = Field({payment: -3, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_COMPANY, group: 6, price: 180, buyingRight: EMPTY_OWNER});
        fields[27] = Field({payment: -3, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_COMPANY, group: 6, price: 190, buyingRight: EMPTY_OWNER});
        fields[28] = Field({payment: -30, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_FEE, group: 0, price: 0, buyingRight: EMPTY_OWNER});
        fields[29] = Field({payment: 100, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_FEE, group: 0, price: 0, buyingRight: EMPTY_OWNER});
        ...
        fields[39] = Field({payment: -60, owner: EMPTY_OWNER, level: 0, fieldType: FIELD_FEE, group: 0, price: 0, buyingRight: EMPTY_OWNER});
    ...
    function _getChance(uint256 _fieldId, uint256 _playerId) private {
        ...
        if (chanceCards[cardNumber] == CHANCE_FEE) {
            player.balance -= CHANCE_FEE_AMOUNT;
            emit BalanceChanged(player.addr, player.balance);
        } else if(chanceCards[cardNumber] == CHANCE_REWARD) {
            player.balance += CHANCE_REWARD_AMOUNT;
            emit BalanceChanged(player.addr, player.balance);
        } else if(chanceCards[cardNumber] == CHANCE_TELEPORT) {
            player.weapon_teleport += 1;
            emit WeaponsChanged(player.addr, player.weapon_teleport, player.weapon_diplomacy);
        } else if(chanceCards[cardNumber] == CHANCE_DIPLOMACY) {
            player.weapon_diplomacy += 1;
            emit WeaponsChanged(player.addr, player.weapon_teleport, player.weapon_diplomacy);
        }
    }
